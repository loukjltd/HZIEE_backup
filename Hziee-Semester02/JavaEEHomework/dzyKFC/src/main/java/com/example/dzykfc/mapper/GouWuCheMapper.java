...
public interface GouWuCheMapper {
    //-------------------------new-----------------------
    //传入weiIdId与caiId 对购物车进行查看 并且保证dingId的状态为2；未完成支付
    @Select("select GouWuChe.* from GouWuChe" +
            " join zhuangTai on GouWuChe.dingId = zhuangTai.dingId" +
            " where weiId = #{weiId} and caiId = #{caiId} and zhuangtaiId =2")
    List<GouWuChe> newDCIdCK(@Param("weiId") int weiId,
